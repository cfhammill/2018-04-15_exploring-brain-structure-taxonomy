...
of effects. Conveniently for us, a large brain science group --
the Allen Institute for Brain Science -- has organized their mouse brain histological atlas in just 
...
The Allen brain taxonomy tree defines structure relations as a recursive subdivision of the
...
equivalent to the structures in our MRI atlas. By unifying our nomenclature to match the Allen 
structures, we can combine our MRI atlas with their taxonomy tree. This allows us to divide the 
mouse brain into a hierarchy of structures. This gives us a vocabulary for higher-order structures 
...
1. Regions are not homogeneous, the structure volume affects variance, with small
...
classes of bayesian models that will encourage pooling between structures.
...
1. A standard hierarchical model fit with `rstanarm`'s `stan_lmer` function.
   ...
   taxonomy.
1. Hierarchical models fit with `rstanarm` that incorporate taxonomy information
   by pooling effects across higher-order structures. Typically by adding extra
   pooled effects across some depth of the tree. Examples explored below.
1. A tree based model where effects diffuse down the tree as a gaussian
   random walk. 
   
This third class of models we've termed effect diffusion trees. In the effect diffusion
tree models the covariate effect at each child node is the effect at their parent plus 
...
all models discussed use a `decov` prior for $\Sigma$, which decomposes the covariance
matrix into a variance vector and a correlation matrix, placing a gamma prior on
the variance and a Lewandowski-Kurowicka-Joe (LKJ) prior on the correlations.
For the effect diffusion model, the prior on $\Sigma$ establishes a prior on the 
change in effect associated with taking a step from a parent to a child node. 

Originally the model was formulated such that each parent node had their own covariance matrix,
unfortunately, this model couldn't be fit due to divergences, so only 
...
simulate some data and try to recover our simulated parameters with the
...
2. A hierarchical `stan_lmer` that has an additional effect of parent structure `y ~ sex + (sex | structure) + (sex | parent) + (1 | ID)`, termed the parental model, `hslmer` in code.
3. The effect diffusion tree, in code as `edt`.
...
3. Mean squared error of the predicted volume, $\frac{1}{N}\sum_{i=1}^N(y_i - \hat{y_i})^2$.
4. Fisher-transformed correlation between the estimated parameters
   and the simulated parameters at the leaves, 
   $atanh(cor(\boldsymbol{\beta_l}, \boldsymbol{\hat{\beta_l}}))$.
   where $\boldsymbol{\hat{\beta_l}}$ is the vector of posterior median effect at the leaves and
   $\boldsymbol{\beta_l}$ is the vector of simulated effects at the leaves.
5. Approximate log-likelihood of the simulated effects given the model. This
   is computed as 
   $\sum_s log \left( \mathcal{N}(\beta_s ~|~ \mathbb{E}(\boldsymbol{\hat{\beta_s}}), sd(\boldsymbol{\hat{\beta_s}})) \right)$ 
...
labels getting over-written, and some from `loo` indicating slightly high pareto-k values
for several of the parental models.

```{r extract_res, cache = TRUE, dependson = "fit_models", warning = FALSE, message = FALSE}
model_fits <- readRDS("simulation_results.rds")
...
After examining the difference posteriors we can make decisions regarding where in the
...
1. The effect diffusion model, in code as `edt`
...
<pre><code class="hljs">## [1] "n_eff / iter looks reasonable for all parameters"
...
## [1] "E-BFMI indicated no pathological behavior"</code></pre>
...
So the effect diffusion tree is performing marginally better than two of the
three `stan_lmer` models. The effect diffusion tree has an edge in both ELPD 
and effect correlation against the flat model, and virtually identical to 
the parental model. The effect diffusion tree has a slightly worse ELPD than 
the grand-parental model. The effect diffusion tree also has more effective 
parameters than the grand-parental model.
...
the Jaccard similarity of each set of elements. The Jaccard similarity is
...
effect diffusion tree and the flat model as well as the effect diffusion tree
and parental model. The grand-parental model was 
most dissimilar from the rest and identified the fewest structures.
...
with grand-parent information recovers only two. Only the parental model identified
the left and right medial preoptic nucleus. Let's examine the posterior distributions
for these structures.

```{r, fig.cap = fig$cap("preoptic", "Posterior distributions for the sex effects in the medial preoptic nuclei across four bayesian models. Pale bands show 50% credible intervals, dark blue lines show posterior medians.")}
list(`Flat model` = real_lmer_res
     , `Parental model` = real_hlmer_res
   , `Grand-parental model` = real_h2lmer_res
   , `Effect Diffusion Tree` = real_edt_res) %>%
  map(~ .$b_post[,c("left Medial preoptic nucleus"
                  , "right Medial preoptic nucleus")]) %>%
  imap( ~ `colnames<-`(.x, paste0(.y, ": ", colnames(.x)))) %>%
  reduce(cbind) %>%
  mcmc_areas     
```

So it looks as though the effect diffusion tree has very broad posteriors for
the preoptic nuclei, the flat model is estimating slightly less broad posteriors
and slightly larger effects. The grand-parental model is estimating narrow posteriors
with smaller effects, where the parental model is estimating relatively large effects
with reasonably narrow posteriors. To examine why this might be the case let's look
at the preoptic nuclei and their immediate neighbours (members of the hypothalamic medial zone)
with both the effect diffusion and parental models:

```{r, fig.cap = fig$cap("hmz_edt", "Posterior distributions for the sex effects in the hypothalamic medial zone in the effect diffusion model. Pale bands show 50% credible intervals, dark blue lines show posterior medians.")}
hmz <-
  FindNode(full_tree, "left Medial preoptic nucleus")$
  parent$
  parent$
  Get("name", filterFun = isLeaf)

mcmc_areas(real_edt_res$b_post[, hmz])
```

```{r, fig.cap = fig$cap("hmz_edt", "Posterior distributions for the sex effects in the hypothalamic medial zone in the parental model. Pale bands show 50% credible intervals, dark blue lines show posterior medians.")}
mcmc_areas(real_hlmer_res$b_post[, hmz])
```

Examining the neighbours of the preoptic nucleus we find the mammillary bodies, which have an 
effect in the opposite direction, so the effect diffusion tree is pooling more strongly between 
the neighbours. 

### Viewing all identified structures

For completeness, let's compare the identified structures for the parental model and the
effect diffusion model, as those two identified the greatest number of structures.

```{r, fig.cap = fig$cap("p_flat", "Posterior distributions for the effect of sex at each leaf node in the parental `stan_lmer` model where the 90% credible does not bound zero. Pale bands show 50% credible intervals, dark blue lines show posterior medians."), fig.height = 12}
...
real_b_posterior <- real_edt_res$b_extra

real_difference_tree$Do(function(n){
  node_num <- match(n$name, real_node_names)
  n$b_diff <-
    real_b_posterior[, paste0("b[", node_num, ",2]") ]
})
...
Region based analyses are really just the beginning. Our acquired images are
